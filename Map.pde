...
private class MapClass {
    ...
    MapClass() {
        ...
        new Frame(
                PVector.mult(startPos, gridSize),
                new PVector(gridSize, gridSize),
                color(60, 60, 60),
                Shape.RECTANGLE
        );
    }
    ...
    private class Node {
        ...
        Frame frame; // frame que representa o nó (usado para debug)
    ...
    public void setPosWeight(PVector pos, int weight) {
        Node node = getNodeFrom(pos);
        if (node != null) {
            node.custo = weight;
            println("novo custo:" + node.custo);
            // atualiza o caminho
    ...
    private void createNode(PVector pos, PVector dir) {
        ...
        // Cria o frame
        node.frame = new Frame(
                new PVector(pos.x * gridSize, pos.y * gridSize),
                new PVector(gridSize, gridSize),
                pathColor,
                Shape.RECTANGLE
        );
    ...
    public void generatePath() {
        ...
        for (int i = 0; i < nPath; i++) {
            ...
            do {
                ...
                curr = temp;
                generatedNodes.add(curr);

            } while ( (i == 0 || nNodes <= maxPathSize) && !curr.pos.equals(endPos));
    ...
    private void reconstructPath(Node current) {
        ...
        while (current.parent != null) {
            ...
            next.frame.backgroundColor = selectedPathColor; // marca o nó como parte do caminho

            nextNodeGrid[(int) current.pos.x][(int) current.pos.y] = next;

        }
        if (current != null) current.frame.backgroundColor = selectedPathColor;
    }
    ...
    public void calculateBestPath(Node start, Node end) {
        ...
        for (int i = 0; i < gridX; i++) {
            for (int j = 0; j < gridY; j++) {
                Node node = path[i][j];
                if (node != null) {
                    node.frame.backgroundColor = pathColor;
                }
            }
        }
